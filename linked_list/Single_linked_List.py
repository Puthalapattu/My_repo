    ...
    def detletAtEnd(self) -> None:
        ...
        if self.head.data is None:
            ...
            if self.length() > 1:
                ...
                self.create_log(deleted_data = self.tail.data, linked_list = linked_list_now, delete_type = 'Delete At End')
                #---------------------------------------------------------------
                self.tail.data = None
                self.tail = current
                current.next = None
    ...
    def deleteAtFront(self) -> None:
        ...
        if self.head.data is None:
            ...
            if self.length() > 1:
                ...
                SLL.__deleted_count += 1
                # call the create_log() method with deleted node and linked list
                self.create_log(deleted_data = self.head.data, linked_list = linked_list_now, delete_type = 'Delete At Front')
                #---------------------------------------------------------------
                current = self.head
                current = current.next
                self.head.data = None
    ...
    def deleteAtposition(self,position:int):
        ...
        if self.head.data is None:
            ...
            if position == self.length():
                ...
                SLL.__deleted_count += 1
                # call the create_log() method with deleted node and linked list
                self.create_log(deleted_data = current.next.data, linked_list = linked_list_now, delete_type = 'Delete At Position')
                #---------------------------------------------------------------
    ...
    def create_log(self, exit_selected:bool = False, deleted_data:any = None, delete_type:str = None, linked_list:list = None, user_requested_log = False) -> None:
        if os.path.exists('txt_files\\linked_list_log.txt'):
            ...
            # ------------ TASK2 STARTS ( when user selected exit option this will run ) ----------------------------------------#
                ...
                MAX LENGTH OF SLL CREATED TODAY : \33 {max_count}\n\n'.format(today_time = str(datetime.now())[11:19], linked_list_now = linked_list, deleted_count = SLL.__deleted_count, max_count = (lambda sll : len(sll) > 0 and max(sll) or '0')(SLL.__size_of_sll))
                )
                    file_object.write('##################################################################################################################################################\n\n')
                return
            # ------------ TASK2 ENDS ----------------------------------------#
...
while True:
    ...
    if ch == 1:
        ...
        if n <= c.length():
            ...
            print('\nPosition {} is not found check the length of sll and try again!.'.format(n),'\n')
        ...
        if c.isEmpty():
            ...
            c.create_log(exit_selected = False, linked_list = linked_list_now, user_requested_log = True)
